...
client.on('ready', () => {
  ...
  BotInfo["Serving"] = {}; // dictionary of user ids tht bot is serving
  client.guilds.cache.forEach(serv => {
    let membs = serv.members.cache.filter(member => member.user.bot === false);
    membs.forEach((value,key)=>{
      BotInfo["Serving"][key] = true;
    });
  });
...
client.on('message', msg => {
  if (!msg.author.bot && !Frozen) {
    if (msg.content.substring(0,Settings.AdminPrefix.length) == Settings.AdminPrefix && Settings.AdminId[msg.author.id.toString()]) { // admin command with prefix
      let cmd = (msg.content.split(/\s/g)[0]).substring(Settings.AdminPrefix.length,msg.content.length).toLowerCase(); // the given command trimmed down to first whitespace
      if (AdminCmds[cmd]) {
        AdminCmds[cmd](msg);
      }
    } else if (msg.content.substring(0,Settings.Prefix.length) == Settings.Prefix) { // basic command with prefix
      let cmd = (msg.content.split(/\s/g)[0]).substring(Settings.Prefix.length,msg.content.length).toLowerCase(); // the given command trimmed down to first whitespace
      if (commandList[cmd]) {
